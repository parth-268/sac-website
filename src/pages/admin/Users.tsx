...
import { 
  Loader2, Search, ShieldAlert, ShieldCheck, 
  User as UserIcon, CalendarClock, ChevronRight, 
  Users, UserCog, Link2, Link2Off
} from "lucide-react";
...
const StatCard = ({ title, value, icon: Icon, color }: any) => (
  <Card className="border-slate-100 shadow-sm">
    <CardContent className="p-4 flex items-center justify-between">
      ...
      <div className={`h-10 w-10 rounded-full flex items-center justify-center ${color} bg-opacity-10`}>
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter(user => {
      ...
      const matchesFilter = 
        ...
        filter === 'team' ? user.is_team_member : true;
      return matchesSearch && matchesFilter;
    });
  }, [users, search, filter]);

  if (isLoading) return <AdminLayout title="User Directory"><Loader2 className="animate-spin mx-auto mt-10 h-8 w-8 text-slate-400" /></AdminLayout>;

  return (
    <AdminLayout title="User Directory" description="Manage user accounts and access.">
      
      {/* 1. Mobile-Friendly Stats Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
        <StatCard title="Total Users" value={stats.total} icon={Users} color="bg-blue-50 text-blue-600" />
        <StatCard title="Admins" value={stats.admins} icon={ShieldAlert} color="bg-purple-50 text-purple-600" />
        <StatCard title="Team Members" value={stats.team} icon={UserCog} color="bg-emerald-50 text-emerald-600" />
      </div>
          ...
          >
            <div className="flex flex-col md:flex-row md:items-center gap-4">
              ...
              <div className="flex items-start justify-between w-full md:w-auto md:min-w-[30%]">
                <div className="flex items-center gap-3 overflow-hidden">
                  <Avatar className="h-10 w-10 border border-slate-100 shrink-0">
                    <AvatarFallback className={`text-xs font-bold ${user.role === 'admin' ? 'bg-purple-100 text-purple-700' : 'bg-slate-100 text-slate-600'}`}>
                      {user.full_name?.slice(0, 2).toUpperCase() || "U"}
                    </AvatarFallback>
                  </Avatar>
                  <div className="flex flex-col min-w-0">
                    <span className="font-semibold text-slate-900 truncate text-sm md:text-base">{user.full_name || "Unknown"}</span>
                    <span className="text-xs text-slate-500 truncate font-mono">{user.email}</span>
                  </div>
                </div>
              ...
              <div className="flex flex-wrap items-center gap-2 md:flex-1">
                {/* Role Badge */}
                {user.role === 'admin' ? (
                  <Badge variant="outline" className="border-purple-200 bg-purple-50 text-purple-700 gap-1 text-[10px] md:text-xs">
                    <ShieldCheck className="w-3 h-3" /> Admin
                  </Badge>
                ) : (
                  <Badge variant="secondary" className="bg-slate-100 text-slate-500 gap-1 text-[10px] md:text-xs">
              ...
              <div className="flex items-center justify-between md:justify-end md:w-[25%] mt-2 md:mt-0 pt-2 md:pt-0 border-t md:border-t-0 border-slate-50">
                <div className="flex items-center gap-1.5 text-xs text-slate-400" title={user.last_sign_in_at || ""}>
                   <CalendarClock className="w-3.5 h-3.5" />
                   {user.last_sign_in_at ? (
                     <span>{formatDistanceToNow(new Date(user.last_sign_in_at), { addSuffix: true })}</span>
                   ) : (
